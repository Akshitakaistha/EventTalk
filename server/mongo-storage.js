...
class MongoStorage {
  ...
  async getFileUploadsBySubmissionIds(submissionIds) {
    try {
      await this.ensureConnection();
      return await FileUpload.find({ submissionId: { $in: submissionIds } }).lean();
    } catch (error) {
      console.error('Error getting file uploads by submission IDs:', error);
      return [];
    }
  }
