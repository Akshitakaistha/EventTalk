...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  /**
   * Function to get the submission details
   */
  app.get("/api/forms/:id/submissions", authenticate, async (req, res) => {
    try {
      ...
      // Convert submissions to plain objects
      const submissionsPlain = submissions.map((s: any) => s.toObject ? s.toObject() : s);
  
      // Get file uploads for all submissions
      const submissionIds = submissionsPlain.map((s: any) => s._id.toString());
      const fileUploads = await storage.getFileUploadsBySubmissionIds(submissionIds);
  
      // Map submissionId to fileUploads
      const filesBySubmission: Record<string, any[]> = {};
      for (const file of fileUploads) {
        const sid = file.submissionId.toString();
        if (!filesBySubmission[sid]) {
          filesBySubmission[sid] = [];
        }
        filesBySubmission[sid].push(file);
      }
  
      // Merge file data with each submission
      const submissionsWithFiles = submissionsPlain.map((submission: any) => ({
        ...submission,
        files: filesBySubmission[submission._id.toString()] || []
      }));

      // Serialize to plain JSON objects to remove Mongoose internals
      const serializedSubmissions = JSON.parse(JSON.stringify(submissionsWithFiles));
  
      return res.json(serializedSubmissions);
    } catch (error) {
  ...
  /**
   * Function to post the form in the submit route 
   */
  ...
  try {
    ...
    const uploadedFiles: any[] = []; // to track file info
    ...
    if (files && files.length > 0) {
      for (const file of files) {
        ...
        if (fieldMatch && fieldMatch[1]) {
          ...
          uploadedFiles.push(fileUploadData); // Add file info to return
        }
      }
    }

    // Return submission with uploaded files
    return res.status(201).json({
      ...submission,
      files: uploadedFiles
    });
 ...
 /**
  * Function to upload the file in the route
  */
...
/**
 * Function to delete the submission in the Application
 */
app.delete("/api/submissions/:id", authenticate, async (req, res) => {
  try {
      const submissionIdStr = req.params.id;
      // Convert submissionId to number if needed
      const submissionId = isNaN(Number(submissionIdStr)) ? submissionIdStr : Number(submissionIdStr);
      ...
      const user = (req as any).user;
      const isOwner = submission.userId === user._id || submission.userId === user._id?.toString();
      ...
      const success = await storage.deleteSubmission(submissionId);
      if (success) {
      ...
      return res.status(500).json({ message: "An error occurred while deleting the submission" });
  }
});
