...
{/*       <div className="min-h-screen bg-gray-50 py-2 px-[12rem] lg:px-[14rem]"> */}
      <div className={`${hasBannerComponent ? 'min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8' : 'min-h-screen bg-gray-50 py-2 px-[8rem] sm:px-[10rem] lg:px-[12rem]'}`}>
              ...
              {/* Special Component Display Area (Banner or PDF) */}
                ...
                >
                  ...
                  {pdfField && (pdfField?.pdfUrl || formValues[pdfField?.id]?.preview) ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-md p-6">
                      <Icons.PdfUpload />
                      <p className="mt-2 text-sm text-gray-500">{pdfField?.label || 'Upload PDF document'}</p>
                      <p className="text-xs text-gray-400 mt-1">{pdfField?.helperText || 'PDF up to 10MB'}</p>
                      <label className="mt-4 cursor-pointer px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                        Upload PDF
                        <input 
                          type="file" 
      ...
      document.body.removeChild(textarea);
                  ...
                  {regularFields.map(field => (
                    <div key={field.id} className={`form-field px-3 ${field.gridColumn === 'half' ? 'md:w-1/2 md:pr-3 md:inline-block' : 'w-full'}`}>
                      {!field.hideLabel && (
                        <>
                          <label className={`${((field.label === 'Mobile Number') || (field.label === 'Resume Upload') || (field.type === 'checkbox') || (field.type === 'toggle')) && 'hidden'} block text-sm font-medium text-gray-700`}>
                            {field.label}
                ...
                <div className="space-y-6">
                  ...
                  {regularFields.map(field => (
                    <div key={field.id} className={`form-field ${field.gridColumn === 'half' ? 'md:w-1/2 md:pr-3 md:inline-block' : 'w-full'}`}>
                      {!field.hideLabel && (
                        <>
                          <label className={`${((field.label === 'Mobile Number') || (field.type === 'checkbox') || (field.type === 'toggle')) && 'hidden'} block text-sm font-medium text-gray-700`}>
                            {field.label}
                            {field.required && <span className="text-red-500 ml-1">*</span>}
                          </label>
                          {field.helperText && <p className="text-xs text-gray-500 mb-2">{field.helperText}</p>}
